...
fn parse_source_tree(source_path: &str, json_output: &mut std::fs::File) {
    ...
    for c_file in &c_files {
        ...
        // Create or open the function_calls.json file for writing
...
fn main() {
    ...
    let path = &args[1];
    let mut json_output = OpenOptions::new()
        .write(true)
        .create(true)
        .truncate(true)  // Delete the file if it already exists and create again
        .open("function_calls.json")
        .expect("Failed to open or create file");
    parse_source_tree(path, &mut json_output);
}
